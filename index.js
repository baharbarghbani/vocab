// =====================
// Vocab Pocket (index.js)
// Persists edits/adds in localStorage
// =====================
...
const VOCAB = [
  ...
  // duplicates you had — we'll optionally remove them below
  { w:"Deteriorate", d:"to become worse" },
...
function dedupeKeepFirst(items){
  // Keep first definition per word (case-insensitive)
  const seen = new Set();
  const out = [];
...
// =====================
const STORAGE_KEY = "vocab_pocket_data_v1";
...
function loadData(){
  try{
    ...
    return parsed
      ...
      .map(x => ({
        w: typeof x.w === "string" ? x.w : "",
        d: typeof x.d === "string" ? x.d : "",
        ...(typeof x.e === "string" && x.e.trim() ? { e: x.e } : {})
      }))
      .filter(x => x.w.trim() && x.d.trim());
  } catch {
    return null;
  }
}
...
// initial seed
const initial = REMOVE_DUPLICATES ? dedupeKeepFirst(VOCAB) : [...VOCAB];

// Data used by the app (loaded from storage if possible)
const DATA = loadData() ?? initial;

// If first run (no saved), persist the initial dataset
...
function edit(e, it){
  ...
  const existingExample = typeof it.e === "string" ? it.e : "";
  const newExample = prompt(`(Optional) Edit example sentence for "${it.w}":`, existingExample);
  ...
  if (newExample === null){
    ...
    if (ex){
      ...
      // if user clears it, remove example field
      delete it.e;
    }
  }
...
function render(){
  ...
  let items = DATA.filter(it => {
    ...
    const ex = normalize(it.e);
    return !q || w.includes(q) || d.includes(q) || ex.includes(q);
  });

  if (sortAZ){
    items.sort((a,b) => normalize(a.w).localeCompare(normalize(b.w)));
  }
...
els.add.addEventListener("click", () => {
  ...
  const def = prompt(`Enter the definition for "${w}":`);
  if (def === null) return;
  ...
  const example = prompt(`(Optional) Enter an example sentence for "${w}":`);
  if (example === null){
    // user cancelled example prompt → still add without example
    DATA.push({ w, d });
  } else {
